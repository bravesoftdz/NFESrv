...
type
  ...
  IServidorMDFe = interface(IServidor)
    ...
    function GerarQRCodeURL(AUrl, AChaveMDFe: WideString): WideString;
    procedure GerarPDF(AMensagem, ARodape: WideString; AXml, ALogo, APdf: TStream);
...
uses
  ...
  ACBrCAPICOM_TLB, WinInet, JwaWinCrypt, distNFeRS_v100, XMLDoc, ACBrDFE,
  ACBrDFeSSL, ACBrDFeCapicom, ACBrDFeWinCrypt;
...
begin
  ...
  CertConf := TDFeSSL.Create;
  try
    CertConf.SSLCryptLib := cryWinCrypt;
    FNumeroSerie := CertConf.SelecionarCertificado;

    Cert         := TDFeWinCrypt(CertConf.SSLCryptClass);
    //Cert         := TDFeCapicom(CertConf.SSLCryptClass).Certificado;

    FChaveAcesso := Cert.CertSubjectName;
    FCnpjTitular := CertConf.CertCNPJ;
    FValidadeFim := FormatDateTime('dd/mm/yyyy hh:nn:ss', Cert.CertDataVenc);
    FValidadeIni := FormatDateTime('dd/mm/yyyy hh:nn:ss', Now);
...
//    FValidadeIni := FormatDateTime('dd/mm/yyyy hh:nn:ss', Cert.ValidFromDate);
    ...
    FNomeTitular := CertConf.CertRazaoSocial;
    FEmissor := CertConf.CertCertificadora;
  finally
    FreeAndNil(CertConf);
  end;
...
function TServidorMDFe.GerarQRCodeURL(AUrl, AChaveMDFe: WideString): WideString;
var
  MDFeUtil: MDFe_Util_Interface;
  xmlResultado, qrCode, msgResultado: WideString;
  iResult: Integer;
begin
  MDFeUtil := MDFe_CoUtil.Create;
  try
    xmlResultado := MDFeUtil.infMDFeSupl( AUrl,
                                          AChaveMDFe,
                                          Certificado.ServidorMDFe.Ambiente + 1,
                                          Certificado.ChaveAcesso,
                                          qrCode,
    ...
    if (iResult <> 6200) then
        raise ExceptionMDFe.Create(iResult, msgResultado);

    //Result := xmlResultado;
    Result := qrCode;

  finally
    SrvNfe.IncluirLogServico(MDFe_GerarQRCodeURL,
                             iResult,
                             'MDFe_GerarQRCodeURL',
                             '',
                             '',
                             msgResultado,
                             '',
                             'Url: ' + AUrl + ' | ChaveMDFe: ' + AChaveMDFe);

    MDFeUtil := nil;
  end;
end;

procedure TServidorMDFe.GerarPDF(AMensagem, ARodape: WideString; AXml, ALogo, APdf: TStream);
var
  MDFeUtil: MDFe_Util_Interface;
  parametros, diretorio, arqXml, arqPdf, arqLogo, strXml: WideString;
  msgResultado: WideString;
  FArq: TFileStream;
  iResult: Integer;
begin
  arqXml := Diretorio + 'xml_'+IntToStr(GetCurrentThread)+'.tmp';
  arqPdf := 'pdf_'+IntToStr(GetCurrentThread)+'.pdf';
  arqLogo := Diretorio + 'pdf_'+IntToStr(GetCurrentThread)+'.jpg';

  MDFeUtil := MDFe_CoUtil.Create;
  try
    diretorio := ExtractFilePath(Application.ExeName) + IncludeTrailingPathDelimiter('MDFE_PDF');
    if not DirectoryExists(Diretorio) then
      MkDir(Diretorio);

    // XML
    FArq := TFileStream.Create(arqXml, fmCreate);
    try
      AXml.Position := 0;
      FArq.CopyFrom(AXml, AXml.Size);
    finally
      FreeAndNil(FArq);
    end;

    // LOGO
    FArq := TFileStream.Create(arqLogo, fmCreate);
    try
      ALogo.Position := 0;
      FArq.CopyFrom(ALogo, ALogo.Size);
    finally
      FreeAndNil(FArq);
    end;

    parametros := '[ARQUIVO=' + arqPdf + '][PASTA=' + diretorio + ']';
    if not AnsiSameText(Trim(AMensagem), EmptyStr) then
      parametros := parametros + '[MENSAGEM=' + AMensagem + ']';
    if not AnsiSameText(Trim(ARodape), EmptyStr) then
      parametros := parametros + '[RODAPE=' + ARodape + ']';

    strXml := MDFeUtil.LeArquivoANSI(arqXml, iResult, msgResultado);

    iResult := MDFeUtil.geraPdfDAMDFE( strXml,
                                       arqLogo,
                                       'N',
                                       parametros,
                                       msgResultado);

    if (iResult > 7901) then
        raise ExceptionMDFe.Create(iResult, msgResultado);

    FArq := TFileStream.Create(arqPdf, fmOpenRead);
    try
      FArq.Position := 0;
      TMemoryStream(APdf).Clear;
      TMemoryStream(APdf).LoadFromStream(FArq);
    finally
      FreeAndNil(FArq);
    end;

  finally
    SrvNfe.IncluirLogServico(MDFe_GerarPDF,
                             iResult,
                             'MDFe_GerarPDF',
                             '',
                             '',
                             msgResultado,
                             '',
                             'AMensagem: ' + AMensagem + ' | Rodapé: ' + ARodape);

    MDFeUtil := nil;

    if FileExists(arqXml) then
      DeleteFile(arqXml);
    if FileExists(Diretorio + arqPdf) then
      DeleteFile(Diretorio + arqPdf);
  end;
end;
...
begin
  ...
  if ServidorNFSe.Certificado.ServidorNFe.Ambiente = 0 then
    //AHttp.URL := 'https://nfe-rs-integracao.sefazvirtual.rs.gov.br/ws/NfeIntegracao/NfeIntegracao.asmx' 06/01/2020  Alterado a URL
    AHttp.URL := 'https://dfe-servico.svrs.rs.gov.br/WS/NFeIntegracao/NFeIntegracao.asmx'
  else
    //AHttp.URL := 'https://nfe-rs-integracao-hmle.sefazvirtual.rs.gov.br/ws/NfeIntegracao/NfeIntegracao.asmx';
    AHttp.URL := 'https://dfe-servico-homologacao.svrs.rs.gov.br/WS/NFeIntegracao/NFeIntegracao.asmx';
...
begin
  CertConf := TDFeSSL.Create;
  try
    CertConf.SSLCryptLib := cryCapicom;
    CertConf.NumeroSerie := ServidorNFSe.Certificado.NumeroSerie;
    CertConf.CarregarCertificado;

    Cert        := TDFeCapicom(CertConf.SSLCryptClass).Certificado;; //CertConf.GetCertificado;
