...
type
  ...
  private
    ...
    procedure MDFeGerarQRCodeURL(AConnection: TIdTCPServerConnection; ACertificado: ICertificadoDigital);
    procedure MDFeGerarPDF(AConnection: TIdTCPServerConnection; ACertificado: ICertificadoDigital);
  public
    { Public declarations }
  ...
  begin
    ...
    try
      ...
      begin
        ...
        scoNFE.Params.Values['USER_NAME']  := iConfig.ReadString('NFeConfig', 'USERNAME', '');
        scoNFE.Params.Values['PASSWORD']   := Decoder64.DecodeString(iConfig.ReadString('NFeConfig', 'PASSWORD', ''));
        CarregaPadrao := False;
     end;
    finally
      FreeAndNil(Config);
...
begin
  ...
  RegistrarComando(MDFe_GerarQRCodeURL, MDFeGerarQRCodeURL);
  RegistrarComando(MDFe_GerarPDF, MDFeGerarPDF);
...
procedure TSrvNfe.MDFeGerarQRCodeURL(AConnection: TIdTCPServerConnection;
  ACertificado: ICertificadoDigital);
var
  Stream: TMemoryStream;
  URL, chaveMDFe, QRCode: string;
begin
  Stream := TMemoryStream.Create;
  try
    URL := AConnection.ReadLn;
    chaveMDFe := AConnection.ReadLn;

    QRCode := ACertificado.ServidorMDFe.GerarQRCodeURL(URL, chaveMDFe);
    AConnection.WriteInteger(Integer(NFe_OK));
    AConnection.WriteLn(QRCode);
  finally
    FreeAndNil(Stream);
  end;
end;

procedure TSrvNfe.MDFeGerarPDF(AConnection: TIdTCPServerConnection;
  ACertificado: ICertificadoDigital);
var
  Mensagem, Rodape: string;
  StreamXml, StreamPDF, StreamLogo: TMemoryStream;
begin
  ...
  try
    Mensagem := AConnection.ReadLn;
    Rodape := AConnection.ReadLn;
    AConnection.ReadStream(StreamXml); // XML
    AConnection.ReadStream(StreamLogo); // XML

    ACertificado.ServidorMDFe.GerarPDF(Mensagem, Rodape, StreamXml, StreamLogo, StreamPDF);

    AConnection.WriteInteger(Integer(NFe_OK));
    StreamPDF.Position := 0;
    AConnection.WriteStream(StreamPDF, True, True);
  finally
    FreeAndNil(StreamLogo);
    FreeAndNil(StreamPDF);
    FreeAndNil(StreamXml);
  end;
end;
