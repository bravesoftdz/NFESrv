...
function TNFSeCampoBom.AssinarNFSe(AXml, AReferencia, ATag,
...
begin
  XmlEnv := '<?xml version="1.0"?>' +
            '<pedidoStatusLote xmlns="http://ws.pc.gif.com.br/" versao="1.0">' +
...
begin
  ...
  ASchemaName := 'http://ws.pc.gif.com.br/';
end;
...
begin
  ...
  try
    ...
    begin
      ...
      RetSt := SeparaDados(SeparaDados(RetWS, 'NFSe', False), 'sit', False);
      if (RetSt = '') then
        RetSt := SeparaDados(RetWS, 'sit', False);
      RetMt := SeparaDados(RetWS, 'motivos', False);
      if (Trim(RetMt) = '') then
